...
interface WalkieTalkiesTrackerProps {
  selectedPeriod: TimePeriod;
}

export const WalkieTalkiesTracker = ({ selectedPeriod }: WalkieTalkiesTrackerProps) => {
  const [checkouts, setCheckouts] = useState<WalkieTalkieCheckout[]>([]);
  const [stats, setStats] = useState<WalkieTalkieStats>({
  ...
  useEffect(() => {
    const fetchWalkieTalkieData = async () => {
      try {
        ...
        const { data: currentCheckouts } = await supabase
          ...
          .eq('station_type', 'walkie_talkies' as any)
          .eq('transaction_type', 'walkie_talkie_checkout' as any)
        ...
        const { data: checkins } = await supabase
          ...
          .eq('station_type', 'walkie_talkies' as any)
          .eq('transaction_type', 'walkie_talkie_checkin' as any)
          .gte('created_at', boundaries.start.toISOString())
          .lt('created_at', boundaries.end.toISOString());
        ...
        const formattedCheckouts: WalkieTalkieCheckout[] = activeCheckouts.map(checkout => {
        ...
        const { data: allCheckouts } = await supabase
          ...
          `)
          .eq('station_type', 'walkie_talkies' as any)
          .in('transaction_type', ['walkie_talkie_checkout', 'walkie_talkie_checkin'] as any)
          .gte('created_at', boundaries.start.toISOString())
          .lt('created_at', boundaries.end.toISOString())
        ...
        if (allCheckouts) {
          ...
          allCheckouts.forEach(transaction => {
            ...
            if (transaction.transaction_type === 'walkie_talkie_checkout') {
              sessionMap.set(key, time);
            } else if (transaction.transaction_type === 'walkie_talkie_checkin' && sessionMap.has(key)) {
        ...
        console.error('Error fetching walkie talkie data:', error);
      } finally {
        setIsLoading(false);
      }
    };

    fetchWalkieTalkieData();
  }, [selectedPeriod]);
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Radio className="h-5 w-5" />
            Walkie Talkies Tracking - {formatTimePeriod(selectedPeriod)}
          </div>
          <Badge 
      ...
      <CardContent className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="text-center p-4 bg-warning/10 rounded-lg">
            <Radio className="h-6 w-6 text-warning mx-auto mb-2" />
        ...
        <div>
          ...
          {checkouts.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p>No walkie talkies currently checked out</p>
              ...
              <Table>
                ...
                <TableBody>
                  {checkouts.map((checkout) => {
                    ...
                    const isLongDuration = durationMinutes > 120; // 2+ hours
