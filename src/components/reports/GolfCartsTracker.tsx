...
interface GolfCartsTrackerProps {
  selectedPeriod: TimePeriod;
}

export const GolfCartsTracker = ({ selectedPeriod }: GolfCartsTrackerProps) => {
  const [checkouts, setCheckouts] = useState<GolfCartCheckout[]>([]);
  const [stats, setStats] = useState<GolfCartStats>({
    ...
    const fetchGolfCartData = async () => {
      try {
        ...
        const { data: currentCheckouts } = await supabase
          ...
          .eq('station_type', 'golf_carts')
          .eq('transaction_type', 'golf_cart_checkout')
        ...
        const { data: checkins } = await supabase
          ...
          .eq('station_type', 'golf_carts')
          .eq('transaction_type', 'golf_cart_checkin')
          .gte('created_at', boundaries.start.toISOString())
          .lt('created_at', boundaries.end.toISOString());
        ...
        const formattedCheckouts: GolfCartCheckout[] = activeCheckouts.map(checkout => {
        ...
        const { data: allCheckouts } = await supabase
          ...
          `)
          .eq('station_type', 'golf_carts')
          .in('transaction_type', ['golf_cart_checkout', 'golf_cart_checkin'])
          .gte('created_at', boundaries.start.toISOString())
          .lt('created_at', boundaries.end.toISOString())
        ...
        if (allCheckouts) {
          ...
          allCheckouts.forEach(transaction => {
            ...
            if (transaction.transaction_type === 'golf_cart_checkout') {
              sessionMap.set(key, time);
            } else if (transaction.transaction_type === 'golf_cart_checkin' && sessionMap.has(key)) {
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Car className="h-5 w-5" />
            Golf Carts Tracking - {formatTimePeriod(selectedPeriod)}
          </div>
          <Badge 
        ...
        <div>
          ...
          {checkouts.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p>No golf carts currently checked out</p>
              ...
              <Table>
                ...
                <TableBody>
                  {checkouts.map((checkout) => {
                    ...
                    const isLongDuration = durationMinutes > 240; // 4+ hours
