-- Update activation functions to track veteran thanking
-- This enhances the existing activation functions to set veteran_thanked_at timestamp

-- Update the activate_group_by_phone function to track veteran thanking
...
BEGIN
    ...
    -- Update attendees.activated_at and veteran_thanked_at for attendees who have RFID tags
    UPDATE attendees a3
    SET 
        activated_at = CASE 
            WHEN activated_at IS NULL THEN NOW() 
    ...
    SELECT 
        ...
        jsonb_build_object(
            ...
            'has_rfid', true,
            'is_veteran', a.is_veteran,
            'veteran_thanked', (a.is_veteran = true AND a.veteran_thanked_at IS NOT NULL)
        )
    FROM attendees a
        ...
        SELECT 
            ...
            ARRAY_AGG(
                jsonb_build_object(
                    ...
                    'is_veteran', a.is_veteran,
                    'veteran_thanked_at', a.veteran_thanked_at,
                    'status', CASE 
...
-- Update the activate_entire_order_by_phone function to track veteran thanking
...
-- Update the activate_remaining_rfids_by_phone function to track veteran thanking
...
BEGIN
    ...
    -- Update attendees.activated_at and veteran_thanked_at for newly activated attendees with RFID
    UPDATE attendees a
    SET 
        activated_at = NOW(),
        veteran_thanked_at = CASE 
