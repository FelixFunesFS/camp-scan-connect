-- Comprehensive RFID cleanup migration
-- This will preserve only the 3 specified keeper RFIDs and clean up all others

-- First, let's identify the keeper RFIDs and their attendee_ids for reference
-- Keeper RFIDs: mocke24098509, mockwwer098, mocke2340985
...
ALTER TABLE rfid_tags DROP CONSTRAINT IF EXISTS unique_active_rfid_per_attendee;

-- Create a unique partial index to ensure only one active/assigned RFID per attendee
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS unique_active_rfid_per_attendee 
