-- Fix ambiguous column references in activate_group_by_phone function UPDATE statements
...
BEGIN
    ...
    -- Also update attendees activated_at timestamp with explicit table alias
    UPDATE attendees a3
    SET activated_at = CASE 
        WHEN activated_at IS NULL THEN NOW() 
        ELSE activated_at 
