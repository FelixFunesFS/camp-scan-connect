...
DROP FUNCTION IF EXISTS public.extract_parking_assignment(jsonb);

-- Create the new function with the correct name and identical logic
CREATE OR REPLACE FUNCTION public.extract_site_location_assignment(custom_fields_data jsonb)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
    ...
    IF site_assignment IS NOT NULL THEN
