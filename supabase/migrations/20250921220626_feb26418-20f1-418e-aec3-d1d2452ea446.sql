-- Fix the lookup_attendees_by_phone function to eliminate Cartesian product
...
BEGIN
    ...
    -- Get order companions (people in same orders but different phones) - Fixed aggregation
    order_companions_agg AS (
        SELECT 
            pa.attendee_order_id,
            ARRAY_AGG(
                jsonb_build_object(
                    'id', oc.id,
                    ...
                    'phone', oc.phone,
                    'meal_plan', oc.meal_plan,
                    'arrival_window', oc.arrival_window,
                    'ticket_type', oc.ticket_type,
                    'waiver_signed', oc.waiver_signed,
                    'rfid_uid', rt2.uid,
                    'activated_at', oc.activated_at,
                    'rfid_activated_at', rt2.activated_at,
                    'rfid_status', rt2.status,
                    'is_activated', CASE WHEN oc.activated_at IS NOT NULL THEN true ELSE false END,
                    'has_rfid', CASE WHEN rt2.uid IS NOT NULL THEN true ELSE false END
                )
            ) as companions
        FROM (SELECT DISTINCT attendee_order_id FROM phone_attendees WHERE attendee_order_id IS NOT NULL AND attendee_order_id != '') pa
    ...
    summary AS (
        SELECT 
            ...
            COALESCE(
                (SELECT ARRAY_AGG(companion) 
                 FROM order_companions_agg oca 
                 CROSS JOIN UNNEST(oca.companions) as companion
                 WHERE oca.attendee_order_id IN (SELECT DISTINCT attendee_order_id FROM phone_attendees WHERE attendee_order_id IS NOT NULL AND attendee_order_id != '')
                ), 
                ARRAY[]::jsonb[]
            ) as companions
